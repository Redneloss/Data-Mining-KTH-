...
public class CLI {
  ...
  @Option(name = "-enhanced", usage = "Use enhanced simulated annealing")
  private boolean ENHANCED = false;

  @Option(name = "-temp_enh", usage = "Initial temperature for enhanced simulated annealing")
  private float TEMPERATURE_ENH = 1;

  @Option(name = "-min_temp_enh", usage = "Minimal temperature for enhanced simulated annealing")
  private float MIN_TEMPERATURE_ENH = 0.1f;

  @Option(name = "-alpha_enh", usage = "Alpha factor for enhanced simulated annealing")
  private float ALPHA_ENH = 0.9f;

  @Option(name = "-iter_enh", usage = "Number of iterations per single temperature for enhanced simulated annealing")
  private int ITER_ENH = 100;

  @Option(name = "-restart", usage = "Turn on SA restarts")
  private boolean RESTART = false;

  @Option(name = "-restart_interval", usage = "Restart SA after T reaches its min. value after this number of rounds")
  private int RESTART_INTERVAL = Integer.MAX_VALUE;
  //-----------------Added by Denys Tykhoglo----------------------------
  ...
  public Config parseArgs(String[] args) throws FileNotFoundException {
    ...
    return new Config().setRandNeighborsSampleSize(randNeighborsSampleSize)
            ...
            .setTempEnh(TEMPERATURE_ENH)
            .setMinTempEnh(MIN_TEMPERATURE_ENH)
            .setAlphaEnh(ALPHA_ENH)
            .setIterEnh(ITER_ENH)
            .setRestart(RESTART)
